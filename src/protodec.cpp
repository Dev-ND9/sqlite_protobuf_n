...
void toJson(Field *field, std::ostream &os, bool showType)
{
    if (!field->subFields.empty())
    {
        ...
        if (dtVal && dtScale)
        {
            ...
            if (scaleVal == 0) // DAYS (e.g. YYYY-MM-DD)
            {
            ...
            else if (scaleVal == 5) // TICKS (e.g. YYYY-MM-DDTHH:MM:SS.fffffff)
            {
        ...
        // 3. Check for Guid signature
        Field* guidLo = field->getSubField(1, WIRETYPE_I64, 0);
        Field* guidHi = field->getSubField(2, WIRETYPE_I64, 0);
        if (guidLo && guidHi && !dtVal && !decVal)
        {
            uint64_t lo = 0, hi = 0;
            getFixed64(&guidLo->value, &lo, 0);
            getFixed64(&guidHi->value, &hi, 0);
        ...
        // --- Standard Object/Array Rendering ---
        os << "{";
    ...
    {
        int64_t number;
        getInt64(&field->value, &number, 0);
        os << number;
    }
    else if (field->wireType == WIRETYPE_I64)
    {
        uint64_t raw_val = 0;
        getFixed64(&field->value, &raw_val, 0);
        os << raw_val; // Fixed: Outputs clean integer instead of e+ notation double
    }
    else if (field->wireType == WIRETYPE_I32)
    {
        float number;
        getFloat(&field->value, &number, 0);
        os << number;
    }
    else
    {
...
static inline int getI64(const Buffer *in, void *out, int64_t index)
{
    ...
    index = index < 0 ? index*sizeof(uint64_t) + length : index*sizeof(uint64_t);
...
static inline int getI32(const Buffer *in, void *out, int64_t index)
{
    ...
    index = index < 0 ? index*sizeof(uint32_t) + length : index*sizeof(uint32_t);
